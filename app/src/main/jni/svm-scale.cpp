...
#define SKIP_ELEMENT\
	...
	while(readline(fp)!=NULL)
	{
		...
		sscanf(p,"%lf",&target);
		y_max = max2(y_max,target);
		y_min = min2(y_min,target);
		...
		while(sscanf(p,"%d:%lf",&index,&value)==2)
		{
			...
			{
				feature_max[i]=max2(feature_max[i],0);
				feature_min[i]=min2(feature_min[i],0);
			}
			
			feature_max[index]=max2(feature_max[index],value);
			feature_min[index]=min2(feature_min[index],value);
