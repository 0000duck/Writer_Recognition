...
int svm_recognition(double* data_in,int nfea,svm_model* model)
{
	int j;
	double a;
	a=10.0;
	svm_node* xx=Malloc(svm_node,nfea+1);
	/*CString path;
	path=GetModuleDirectory();
	path+="\\OCRSYS\\model.dat";
	model1=svm_load_model(path);*/

		for(j=0;j<nfea;j++)
		{
			xx[j].index=j+1;
			xx[j].value=data_in[j];
		}
		xx[nfea].index=-1;
		a=svm_predict(model,xx);
		j=(int) a;
